...
import com.byl.qrobotor.bean.jokebean.DanmakuBean;
...
import com.byl.qrobotor.util.DanmuUtil;
...
public class LivePlayModelImpl implements ILivePlayModel {
    /**********************
     * 以下是弹幕Socket连接相关
     *****************/
    ...
    @Override
    public void getLiveDetail(String live_type, String live_id, String game_type, final BaseOnLoadingListener<LiveDetailBean> listener) {
        RetrofitHelper.getLiveHelper()
                ...
                .subscribe(new HttpSubscriber<LiveDetailBean>() {
                    ...
                    public void _onNext(LiveDetailBean liveDetailBean) {
                        listener.onSuccessed(liveDetailBean);
                    }
    ...
    @Override
    public void getDanmuDetail(String roomid, String live_type, final BaseOnLoadingListener<LivePandaBean> listener) {
        if (live_type.equals("panda")) {
            RetrofitHelper.getPandaHelper()
...
//                            view.showError("弹幕服务器接口已过期，请刷新直播列表！");
                        ...
                        public void onNext(LivePandaBean detailPandaBean) {
                            if (detailPandaBean.getErrno() == 0) {
                                listener.onSuccessed(detailPandaBean);
                            } else {
                                listener.onFailed(detailPandaBean.getErrmsg());
                            }
                        }
                    });
        } else {
            listener.onFailed("直播平台：" + live_type + "！不是熊猫TV的弹幕池！");
        }
    }

    @Override
    public void connectToChatRoom(String live_id, final LivePandaBean pandaBean, final BaseOnLoadingListener<DanmuBean> listener) {
        chatRoomList = pandaBean.getData().getChat_addr_list();
        if (chatRoomList == null || chatRoomList.size() == 0) {
            listener.onFailed("无法连接至弹幕服务器！");
            return;
        }
            ...
            public void subscribe(FlowableEmitter<String> e) throws Exception {
                ...
                .subscribe(new DefaultSubscriber<String>() {
                    ...
                    public void onNext(String result) {
                        //连接成功
                        if (isConnectSuccess) {
                            runHeartDanmuOnRxJava(listener);
                        }
                    }
     ...
     * 2.接收弹幕
     */
    private void runHeartDanmuOnRxJava(final BaseOnLoadingListener<DanmuBean> listener) {
        //心跳包
...
//                        view.showError(e.getMessage());
     ...
     * @param danmu
     */
    private void parseDanmu(final String danmu, final BaseOnLoadingListener<DanmuBean> listener) {
